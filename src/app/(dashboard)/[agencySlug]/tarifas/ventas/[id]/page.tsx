...
export default async function EditTarifaVentaPage({
  params,
}: {
  ...
  const [tariff, providers, pensionRegimes, roomTypes, foodTypes, excursionCodes, transferSegments, ticketSegments] =
    await Promise.all([
      prisma.tariff.findUnique({ where: { id, agencyId: agency.id } }),
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Editar Tarifa de Venta</h1>
        <p className="text-muted-foreground text-sm">{providers.find((p) => p.id === tariff.providerId)?.fantasyName}</p>
      </div>
      <CostTariffForm
        mode="tariff"
        agencyId={agency.id}
